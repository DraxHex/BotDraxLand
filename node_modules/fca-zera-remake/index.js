...
if (!fs.existsSync(__dirname + '/database.json')) {
    fs.writeFileSync(__dirname + "/database.json",
        JSON.stringify({
            Account: "",
            Password: "",
            TempState: "",
...
global.Fca = new Object({
    ...
    Data: new Object({
        ObjFastConfig: {
            ...
            "AutoLogin": false,
            "BroadCast": true,
            "AuthString": "ESX5 RIR5 EGNI IJJK X4K3 KDG7 PXOI F6XD",
            "EncryptFeature": true,
    ...
    AutoLogin: async function() {
        ...
        var Email = global.Fca.Require.Database.Account.replace(RegExp('"', 'g'), ''); //hmm IDK
        var PassWord = global.Fca.Require.Database.Password.replace(RegExp('"', 'g'), '');
        login({ email: Email, password: PassWord }, async(error, api) => {
            ...
            try {
                Database.TempState = api.getAppState()
                global.Fca.Require.fs.writeFileSync(__dirname + "/database.json",
...
try {
    ...
    if (global.Fca.Require.fs.existsSync('./FastConfigFca.json')) {
        try {
            if (!DataLanguageSetting.AuthString || global.Fca.Require.utils.getType(DataLanguageSetting.AuthString) != 'String') {
                DataLanguageSetting.AuthString = "ESX5 RIR5 EGNI IJJK X4K3 KDG7 PXOI F6XD | https://i.imgur.com/RAg3rvw.png Please remove this !, Recommend If You Use getUserInfoV2"; //example pls
    ...
    for (let i in DataLanguageSetting) {
        if (Boolean_Fca.includes(i)) {
            if (global.Fca.Require.utils.getType(DataLanguageSetting[i]) != "Boolean") 
                return logger.Error(i + " Is Not A Boolean, Need To Be true Or false !", 
                    function() { process.exit(0) });
            else continue;
        } else if (String_Fca.includes(i)) {
            if (global.Fca.Require.utils.getType(DataLanguageSetting[i]) != "String") 
                return logger.Error(i + " Is Not A String, Need To Be String!",
                 function() { process.exit(0) });
            else continue;
        } else if (Number_Fca.includes(i)) {
            if (global.Fca.Require.utils.getType(DataLanguageSetting[i]) != "Number") 
                return logger.Error(i + " Is Not A Number, Need To Be Number !", 
                    function() { process.exit(0) });
            else continue;
        }
    }
...
var utils = global.Fca.Require.utils,
    ...
    Database = global.Fca.Require.Database,
        ...
        default:
            {
                ...
                res.write(ClassicHTML(global.Fca.Require.FastConfig.HTML.UserName, "IPHONE 14PRO MAX", global.Fca.Require.FastConfig.HTML.MusicLink));
            }
    }
    res.end();
})
        ...
        return utils
            ...
            .then(function( /** @type {{ headers: any; }} */ res) {
                ...
                if (!headers.location) {
                    ...
                    throw { error: Language.InvaildAccount };
                ...
                if (headers.location.indexOf('https://www.facebook.com/checkpoint/') > -1) {
                    ...
                    return utils
                        ...
                        .then(async function( /** @type {{ body: any; }} */ res) {
                            if (!Database.ThroughAcc) {

                                Database.ThroughAcc = email
                                global.Fca.Require.fs.writeFileSync(__dirname + "/database.json",
                                    JSON.stringify(Database, null, "\t")
                                );
                            } else {
                                if (String(Database.ThroughAcc).replace(RegExp('"', 'g'), '') != String(email).replace(RegExp('"', 'g'), '')) {
                                    Database.ThroughAcc = email
                                    global.Fca.Require.fs.writeFileSync(__dirname + "/database.json",
                                        JSON.stringify(Database, null, "\t")
                                    );
                                    if (Database.Through2Fa) {
                                        Database.Through2Fa = ""
                                        global.Fca.Require.fs.writeFileSync(__dirname + "/database.json",
                                            JSON.stringify(Database, null, "\t")
                                        );
                                    ...
                                    case true:
                                        {
                                            try {
                                                ...
                                                async function EnterSecurityCode() {
                                                    try {
                                                        var Through2Fa = Database.Through2Fa;
                                                        if (Through2Fa) {
                                                    ...
                                                    try {
                                                        ...
                                                        var rtPromise = new Promise((resolve, reject) => {
                                                            prResolve = resolve;
                                                            prReject = reject;
                                                        });
...
async function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
    ...
    try {
        if (appState) {
            ...
            switch (Database.FBKEY !== "") {
                case true:
                    {
                        process.env.FBKEY = Database.FBKEY
                    }
                    break;
                case false:
                    {
                                ...
                                case "Object":
                                    {
                                        try {
                                            ...
                                            if (process.env.Backup != undefined && process.env.Backup) {
                                                ...
                                                try {
                                                    if (Database.Backup !== "") {
                                                        return await backup(Database.Backup, globalOptions, callback, prCallback);
                                                    } else {
    ...
    mainPromise = mainPromise
        ...
        .then(function( /** @type {{ body: any; }} */ res) {
            var html = res.body,
                Obj = buildAPI(globalOptions, html, jar);
            ctx = Obj.ctx;
            api = Obj.api;
    ...
    mainPromise
        .then(function() {
            ...
            Fetch('https://raw.githubusercontent.com/ZeraKage/Fca-Zera-Remake/main/package.json').then(async( /** @type {{ body: { toString: () => string; }; }} */ res) => {
                const localVersion = JSON.parse(readFileSync('./node_modules/fca-zera-remake/package.json')).version;
                if (Number(localVersion.replace(/\./g, "")) < Number(JSON.parse(res.body.toString()).version.replace(/\./g, ""))) {
                    log.warn("[ FCA-ZERA ] •", getText(Language.NewVersionFound, JSON.parse(readFileSync('./node_modules/fca-zera-remake/package.json')).version, JSON.parse(res.body.toString()).version));
                    if (global.Fca.Require.FastConfig.AutoUpdate == true) {
                        log.warn("[ FCA-ZERA ] •", Language.AutoUpdate);
                        try {
                            execSync('npm install fca-zera-remake@latest', { stdio: 'inherit' });
...
function setUserNameAndPassWord() {
    ...
    let localbrand2 = JSON.parse(readFileSync('./package.json')).version;
    console.clear();
    console.log(figlet.textSync('ZERA', { font: 'ANSI Shadow', horizontalLayout: 'default', verticalLayout: 'default', width: 0, whitespaceBreak: true }));
                ...
                case true:
                    {
                        if (global.Fca.Require.FastConfig.ResetDataLogin) return setUserNameAndPassWord();
                            ...
                            try {
                                if (Database.TempState) {
                                    try {
                                        loginData.appState = JSON.parse(Database.TempState);
                                    } catch (_) {
                                        loginData.appState = Database.TempState;
                                    }
                                    Database.TempState = ""
                                    global.Fca.Require.fs.writeFileSync(__dirname + "/database.json",
                                        JSON.stringify(Database, null, "\t")
                            ...
                            try {
                                if (Database.Account !== "" && Database.Password !== "") return loginHelper(loginData.appState, loginData.email, loginData.password, globalOptions, callback, prCallback);
