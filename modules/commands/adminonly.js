            ...
            module.exports.run = async function ({ api, event , args }) {
                ...
                if (!Data2a.some(item => item.Misc == event.threadID)) { Data2a.push(thisThread);writeFileSync(pathData, JSON.stringify(Data2a, null ,4), "utf-8");}
                    ...
                    return api.sendMessage("Đã Bật Chế Độ AdminOnly Thành Công ! chế độ chỉ QTV box mới dùng được",event.threadID);
                } 
                else if (thisThread.Status == 2) {
                    thisThread.Status = 1;
                        writeFileSync(pathData, JSON.stringify(Data2a, null ,4), "utf-8");
                    return api.sendMessage("Đã Tắt Chế Độ AdminOnly Thành Công ! Thành viên đều được sử dụng bot",event.threadID);
                }
            }; // ọe