...
module.exports.config = {
    ...
    usages: "<(🍇, 🍉, 🍏, ➐, 🍓, 🍑,) 'số tiền cược'> lưu ý số tiền cược phải trên 50$",
    cooldowns: 5,
  };
 ...
 module.exports.run = async function({ api, event, args, Currencies, getText, permssion }) {
    try {
      ...
      var slot4 = slotItems[Math.floor(Math.random() * slotItems.length)];
      // ARGS
      let content = args[0];
      var content1;
      if (content =='nho' || content == '🍇') {
      ...
      else if (content =='dưahấu' || content == "🍉" ) {
        content1 = 'duahau';
      }
        ...
        return api.sendMessage(`Sai định dạng\n${global.config.PREFIX}${this.config.name} <(🍇, 🍉, 🍏, ➐, 🍓, 🍑,) 'số tiền cược'> lưu ý số tiền cược phải trên 50$`, threadID, messageID);
      }
      // request
      ...
      api.sendMessage({
        ...
        setTimeout(() => {
          ...
          //console.log(check);
          //console.log(icon1 + icon2 + icon3 + icon4);
          if (check >= 0 || check2 == true) {
            return api.sendMessage({
              body: `🌸Quay được: ${icon1} | ${icon2} | ${icon3}\n🌸chúc mừng bạn đã thắng và nhận được +${moneyBet * 3}$`,
              attachment: listimg
