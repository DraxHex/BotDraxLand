module.exports.config = {
  ...
  version: "1.0.5",
  hasPermssion: 0,
  credits: "CatalizCS",//Mod by H.Thanh
...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
  ...
  if (args[1] && isNaN(args[1]) || parseInt(args[1]) <= 0) return api.sendMessage("𝗧𝗢𝗣 - Thông tin độ dài list phải là một con số và không dưới 0", event.threadID, event.messageID);
  ...
  return Math.floor((Math.sqrt(1 + (4 * point) / 3) + 1) / 2);
    ...
    let all = await Currencies.getAll(['userID', 'exp']);
        ...
        let num = 0;
               let msg = {
          body: '𝗧𝗢𝗣 - Top 15 người dùng có level cao nhất server',
  ...
  if (args[0] == "thread") {
    ...
    for(const dataThread of threadList) {
      if (i == option) break;
      msg += `=> ${i+1}/ ${(dataThread.threadName)||"Không tên"}\n=> ID: [${dataThread.threadID}]\n=> Số tin nhắn: ${dataThread.messageCount}\n\n`;
      i+=1;
    }
